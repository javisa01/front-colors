...
/**
 * `/online/daily` — RETIRADA. Solo redirige.
 *
 * ## Qué era
 *
 * La antesala del reto diario: cuántos intentos quedan, la mejor puntuación de
 * hoy, la cuenta atrás al cierre, las reglas y el botón de jugar. Se entraba
 * con `?group=<id>` porque hay un reto por grupo.
 *
 * ## Por qué ya no está
 *
 * Enseñaba **los mismos datos que la ficha del grupo**, una pantalla más
 * adentro: los intentos, la mejor puntuación y el nombre del grupo estaban ya
 * en `/online/groups/[id]`. El resultado era un salto obligatorio entre dos
 * pantallas casi iguales antes de poder jugar, y la clasificación —que es la
 * razón de estar en un grupo— quedaba a dos toques. Toda su función vive ahora
 * en la ficha del grupo, que es de donde el reto es.
 *
 * ## Qué sigue redirigiendo aquí
 *
 * Enlaces guardados y recargas en web. Con grupo se manda a la ficha de ese
 * grupo; sin grupo, a la lista, que es lo mismo que hacía la pantalla vieja en
 * su estado «este reto es de un grupo».
 *
 * `/online/daily/play` **no se ha tocado**: la partida sigue donde estaba.
 *
 * ## Cómo recuperarla
 *
 * La implementación entera está debajo, comentada línea a línea. Para volver a
 * ponerla en pie: borrar este componente y quitar el `// ` del principio de
 * cada línea del bloque de abajo. Lo que además habría que revisar:
 *
 *  - `app/online/groups/[id]/index.tsx` — su botón de jugar apunta directo a
 *    `/online/daily/play`; volvería a apuntar aquí.
 *  - `app/online/daily/play.tsx` — `dailyHref`, el destino de la vuelta.
 *  - `app/online/index.tsx` — los dos `router.push` del hub.
 *  - `app/online/_layout.tsx` — la pantalla sigue declarada, no hace falta nada.
 */
export default function DailyRedirect(): ReactElement {
  const { group } = useLocalSearchParams<{ group?: string }>();
  const groupId = Array.isArray(group) ? group[0] : group;

  if (groupId) {
    return (
      <Redirect href={{ pathname: "/online/groups/[id]", params: { id: groupId } }} />
    );
  }
  return <Redirect href="/online/groups" />;
}

// ===========================================================================
// La antesala del reto diario, retirada. Ver la nota de arriba.
// ===========================================================================
//
